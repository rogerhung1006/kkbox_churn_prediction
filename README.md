...
Partial dependence plots (PDP) show the dependence between the target variable and a set of features of interest, marginalizing over the values of all other features (the complement features). PDP can also show the type of relationship, such as a step function, curvilinear, linear and so on. From Figure 1-3, the longer the tenure, the lower the probailility that a user will churn. This trend goes up to roughly 2500 days, then flattens after that point. For those who have a longer time before making his or her first transaction, the model predicts on average a high probability of churn. It comes with no surprise that the user who have the auto renewal service activated have lower chance of churning. Interestingly, the predicted prbability of churning plummets at the avergae payment of 150 NTD, and bounces back after that. The further examination reveals that 150 NTD is the monthly price for those who are willing to activate the auto renewal service. Bearing this information in mind, I would say the result then makes much sense to me. The partial dependence function at a particular feature value represents the average prediction if we force all data points to assume that feature value. The main advantage is that lay people usually understand the idea of PDPs quickly.
<br>
## Survival Analysis 
